...
EmberSteps.prototype.transform = function(ast) {
  ...
  walker.visit(ast, function(node) {
    ...
    const yieldedAs = program.blockParams && program.blockParams.length && program.blockParams[0];
    if (!yieldedAs) {
      return;
    }
    ...
    childSteps.forEach(function(childNode, index) {
      ...
      if (childNode.inverse) {
        const inverseBlockPair = plugin.buildBooleanPair('hasInactiveState', true);
        childNode.hash.pairs.push(inverseBlockPair);
      }
    ...
    if (!currentStep) {
      ...
      const currentStepPair = plugin.buildStringPair('currentStep', firstStepName);
      node.hash.pairs.push(currentStepPair);
    }

    // Add the step count to the step manager
    const stepCountPair = plugin.buildStringPair('stepCount', childSteps.length.toString());
